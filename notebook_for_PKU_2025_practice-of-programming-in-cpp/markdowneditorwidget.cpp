...
void MarkdownEditorWidget::openMarkdownFile()
{
    ...
    QRegularExpressionMatchIterator it = imageRegex.globalMatch(content);
    while (it.hasNext()) {
        QRegularExpressionMatch match = it.next();
        QString fullMatch = match.captured(0); // 整个 ![](...) 语法
        QString imagePath = match.captured(1); // 括号内路径

        // 查找用户名部分，并替换
        // 假设旧路径形如：![](旧用户名/picture/...)
        int slashIndex = imagePath.indexOf('/');
        if (slashIndex != -1) {
            QString newPath = username + imagePath.mid(slashIndex); // 保留后缀部分
            QString imageFileName = QFileInfo(imagePath).fileName();
            QString newMarkdown = QString("![%1](%2)").arg(imageFileName, newPath);
            newContent.replace(fullMatch, newMarkdown);
        }
    }
    ...
    updatePreview();
    // ✅ 这里初始化图片路径
    oldImagePaths = extractImagePaths(newContent);
